...
public class MockXKMSWebService implements XKMSPortType {
	...
	private RegisterResult processRequest(RequestType request) {
		...
		for (Object object : request.getProcessInfo().getReasonAndReasonCodeAndPublish()) {
			if (object instanceof AttributeCertificate) {
				attributeCertificate = (AttributeCertificate) object;
			}
		}
		ValidityIntervalType validityInterval = attributeCertificate.getValidityInterval();
